...
public class ShareFileWithUser extends Navigation {

	static WebDriver driver;
	static Actions actions;

	private static Logger log = Logger.getLogger(ShareFileWithUser.class.getName());
	public static String Home_Page = "https://app.solixecsdev.com/login";
	public ShareFileWithUser(WebDriver driver) {
		super(driver);
		this.driver = driver;
	...
	public static void share_data_with_user_edit_permission()
	{
		...
		try
		{
			...
			WebElement my_element = new WebDriverWait(driver, 20).until(ExpectedConditions.presenceOfElementLocated(By.xpath("//div[@class='multiselect multiselect--above multiselect_noOptions']")));

			my_element.click();
			//my_element.sendKeys("t");
			
			driver.findElement(By.xpath("//div[@class='multiselect multiselect--active multiselect--active multiselect_noOptions']")).sendKeys("t");
			 */
			
			//input[@placeholder='Search Users or Groups']
	...
	//-------------------------------------------------------multiple Files folders share with edit permissions confirmation-------------------------------------------------------
		...
		public static void share_data_with_user_edit_permission_confirmation()
		{
			...
			WebDriverWait wait212 = new WebDriverWait(driver, 60);
			wait212.until(
					ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='row form-group mb-2']")));
			...
			Thread.sleep(2000);
				...
				for(int i=0;i<each_folder_li_tag.size();i++)
				{
					...
					if(each_li.contains("Share +") || each_li.contains("Add/View Comment") || each_li.contains("Add/View Tags") || each_li.contains("External Sharing") || each_li.contains("Copy / Move") || each_li.contains("Download") || each_li.contains("Add Reminders") || each_li.contains("Rename") || each_li.contains("Trash"))
					{
