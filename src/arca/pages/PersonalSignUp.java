...
public class PersonalSignUp extends Navigation {

	WebDriver driver;
 	
private static Logger log = Logger.getLogger(PersonalSignUp.class.getName());
public static String Home_Page = "https://arcaqa.solix.com/?#/sign-up/personal";
	public PersonalSignUp(WebDriver driver) {
		super(driver);
		this.driver = driver;
		//driver.manage().timeouts().implicitlyWait(60,TimeUnit.SECONDS).pageLoadTimeout(60, TimeUnit.SECONDS);
		ATUReports.setWebDriver(driver);
		ATUReports.setTestCaseReqCoverage("ARCA Personal Signup Page");
		setIndexPageDescription();
		setAuthorInfoForReports();
		log.info("--------------------Entered Individual Personal Signup Page--------------------");
		captureScreenShot(ScreenShotsFilePath + "Individual_Personal_SignupPage_success.png");
	}
	 ...
	 private void setIndexPageDescription() {
	        ATUReports.indexPageDescription = "Individual Personal Signup Page";
	 }
	...
	public void individual_personal_signup() throws Exception
	{
		try {
        ...
        ind_per_signup(generic_email, first_name, last_name, password, confirm_password, official_generic_email, incorrect_password, password_greater_than_12);
	...
	public void ind_per_signup(String signup_email, String signup_first_name, String signup_last_name, String signup_password, String signup_confirm_password, String official_email, String incorrect_signup_password, String signup_password_greter_than_12) throws Exception 
	{
		try {
		...
		Select personal_plan_select = new Select(getPageObject("personal_signup_form_dropdown"));
		WebElement selected_value = personal_plan_select.getFirstSelectedOption();
		String personal_default_value = selected_value.getText();
		System.out.println(personal_default_value);
		Assert.assertTrue(personal_default_value.contains("Individual - Free"));		
		log.info("individual plans - Personal Signup page default selected drop down value is - **Individual - Free**");
		Thread.sleep(2000);
		...
		*/		
		...
		log.info("Please provide a generic email alert displyed");
		
		String password_notmatch_alert=getPageObject("password_not_match_alert").getText();
		Assert.assertTrue(password_notmatch_alert.contains("The password confirmation does not match"));
		log.info("The password confirmation does not match alert displyed");
		...
		*/
		...
		ATUReports.add("Individual personal signup successfull","Individual personal signup",LogAs.PASSED,new CaptureScreen(ScreenshotOf.BROWSER_PAGE));
			...
			System.out.println("errror at: ");
			e.printStackTrace();
			log.error("Error in finding message in personal signup",e);	
	    	log.fatal(e.getMessage(),e);
	    	captureScreenShot(ScreenShotsFilePath + "Individual_Personal_SignupPage-PageError.png");
	    	ATUReports.add("Individual personal signup failed","Individual personal signup",LogAs.FAILED,new CaptureScreen(ScreenshotOf.BROWSER_PAGE));
		}
